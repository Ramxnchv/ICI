...
public class RuleEngine extends RuleEngineObservable{
	...
	public RuleEngine(String engineName, String rulesFile, HashMap<String,Action> map) {
	...
	public MOVE run(Game game)
	{
		try {
			...
			if(actionFact!=null)
			{
				...
				Action gameAction = map.get(actionFact.getSlotValue(Action.ID_SLOT).toString());
	...
	private Fact findActionFact() throws JessException
	{
		...
		while(it.hasNext()){ 
		    ...
		    if(nombre.startsWith("MAIN::"+Action.FACT_NAME))
		    	return dd;
		}
		return null;
	}
